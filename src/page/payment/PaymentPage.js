    ...
    if (payType === "trans") {
      ...
      tossPayments
        .requestPayment("카드", {
          ...
          successUrl:
            window.location.origin + "/successpage?id=" + id + "&type=trans", // 결제 성공 시 이동할 페이지(이 주소는 예시입니다. 상점에서 직접 만들어주세요.)
    ...
    // 호텔 상품이면 아래의 코드가 실행
      ...
      tossPayments
        .requestPayment("카드", {
          ...
          successUrl:
            window.location.origin + "/successpage?id=" + id + "&type=hotel", // 결제 성공 시 이동할 페이지(이 주소는 예시입니다. 상점에서 직접 만들어주세요.)
          failUrl: window.location.origin + "/fail", // 결제 실패 시 이동할 페이지(이 주소는 예시입니다. 상점에서 직접 만들어주세요.)
        })
        // ------결제창을 띄울 수 없는 에러 처리 ------
        // 메서드 실행에 실패해서 reject 된 에러를 처리하는 블록입니다.
